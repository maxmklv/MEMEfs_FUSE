...
int memefs_create(const char* path, mode_t mode, struct fuse_file_info* fi) {
    ...
    // Load directory
    if (lseek(img_fd, superblock.directory_start * BLOCK_SIZE, SEEK_SET) == -1) {
        perror("lseek (directory)");
        return -EIO;
    }
    if (read(img_fd, directory, sizeof(directory)) != sizeof(directory)) {
    ...
    // Load FAT
    if (lseek(img_fd, superblock.main_fat * BLOCK_SIZE, SEEK_SET) == -1) {
        perror("lseek (FAT)");
        return -EIO;
    }
    ...
    // Reserve a block in the FAT for the file's data
    ...
    if (fat_block == 0) {
        fprintf(stderr, "No free FAT block found.\n");
        return -ENOSPC;
    }

    // Set up new directory entry
    memset(new_entry, 0, sizeof(memefs_dirent_t));
    strncpy((char*)new_entry->filename, path + 1, MAX_FILENAME_LEN - 1); // skip '/'
    new_entry->filename[MAX_FILENAME_LEN - 1] = '\0'; // ensure null termination
    ...
    uint8_t bcd_time[8];
    generate_memefs_timestamp(bcd_time);
    memcpy(new_entry->bcd_timestamp, bcd_time, 8);

    // Write updated FAT
    if (lseek(img_fd, superblock.main_fat * BLOCK_SIZE, SEEK_SET) == -1) {
        perror("lseek (write FAT)");
        return -EIO;
    }
    ...
    printf("Created file '%s' at FAT block %u\n", path, fat_block);
    return 0;
}
